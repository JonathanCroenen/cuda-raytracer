...
#define CHECK_CUDA_ERRORS(val)                                                           \
  rtx::utils::CheckCudaErrors((val), #val, __FILE__, __LINE__)

namespace rtx::utils {

inline void CheckCudaErrors(
    cudaError_t result, const char* const func, const char* const file, const int line) {
