...
struct vec3 {
    ...
    CPU_GPU_FUNC inline static vec3<T> cross(const vec3<T>& v1, const vec3<T>& v2) {
        return vec3<T>(v1.y * v2.z - v1.z * v2.y, -(v1.x * v2.z - v1.z * v2.x),
                       v1.x * v2.y - v1.y * v2.x);
    }
    ...
    GPU_FUNC inline static vec3<T> random(curandState* rand_state) {
        return vec3<T>(curand_uniform(rand_state), curand_uniform(rand_state),
    ...
    inline static vec3<T> random() {
        return vec3<T>(rand() / (RAND_MAX + 1.0), rand() / (RAND_MAX + 1.0),
                       rand() / (RAND_MAX + 1.0));
    }
