...
math::vec3<float>* Renderer::render(std::unique_ptr<Camera> camera,
                                    std::unique_ptr<Scene> scene) {
  render_kernel<<<_num_blocks, _num_threads>>>(this, camera.get(), scene.get());
  CHECK_CUDA_ERRORS(cudaGetLastError());
  CHECK_CUDA_ERRORS(cudaDeviceSynchronize());
...
GPU_FUNC math::vec3<float> Renderer::trace(
    const Ray& ray, Camera* camera, Scene* scene, curandState* rand_state) {
  Ray current_ray = ray;
  ...
  for (int i = 0; i < _max_depth; i++) {
    ...
    if (scene->intersect(current_ray, 0.001f, 10000.0f, record)) {
      Ray scattered;
      vec3 attenuation;
      if (record.material->scatter(current_ray, record, attenuation, scattered, rand_state)) {
