...
std::unique_ptr<Scene> Scene::create() { return std::unique_ptr<Scene>(new Scene()); }
...
void Scene::build() {
  ...
  CHECK_CUDA_ERRORS(cudaMemcpy(
      _cuda_objects, _objects.data(), sizeof(Object) * _num_objects, cudaMemcpyHostToDevice));

  _num_materials = _materials.size();
  CHECK_CUDA_ERRORS(cudaMalloc((void**)&_cuda_materials, sizeof(Material) * _num_materials));
  CHECK_CUDA_ERRORS(cudaMemcpy(_cuda_materials,
                               _materials.data(),
                               sizeof(Material) * _num_materials,
